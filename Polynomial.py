...
class Polynomial:
    ...
    def divide( numerator, denominator ):
        ...
        field = denominator.coefficients[0]
    ...
    def __xor__( self, exponent ):
        if self.is_zero():
            return Polynomial([])
        if exponent == 0:
            return Polynomial([self.coefficients[0].one()])
        acc = Polynomial([self.coefficients[0].one()])
    ...
    def test_colinearity( points ):
        """
        Czy 3 punkty leza na tej samej PROSTEJ - dlatego degree == 1
        """
